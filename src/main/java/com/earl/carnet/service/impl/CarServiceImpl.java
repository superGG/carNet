		...
		implements CarService
{
	...
	// 缓存车辆状态信息
	private static Cache CAR_CACHE = EhCacheHelper.getCacheManage()
			.getCache("car");

	// 临时车辆信息缓存
	private static Cache TEM_CAR = EhCacheHelper.getCacheManage()
			.getCache("tem_car");
	...
	private void monitorTemperature(Car model, Car model_data) {
		if (model.getTemperature() != model_data.getTemperature()) { // 幂等处理
			if (model.getTemperature() > model_data.getTemperature()
				...
				if ((model.getTemperature() % 5 < model_data.getTemperature()
						% 5
						|| (model.getTemperature() > 100
								&& model.getTemperature() < 105))) { // 避免多次发送信息，每升高5个单位的温度就通知车主一次
					User user = userService.findOne(model_data.getUserId());
					String content = null;
					if (model_data.getPlateNumber() != null) {
						content = "尊敬的" + user.getUsername() + ": 您好，您的车牌号为"
								+ model_data.getPlateNumber() + " 的车辆当前温度为"
								+ model.getTemperature() + "摄氏度。温度过高，请注意行驶。";
					} else {
						content = "尊敬的" + user.getUsername() + ": 您好，您的车架号为"
								+ model_data.getVin() + " 的车辆当前温度为"
								+ model.getTemperature() + "摄氏度。温度过高，请注意行驶。";
					}
					logger.info("-----content:" + content);
	...
	private void monitorOil(Car model, Car model_data) {
		if (model.getOil() != model_data.getOil()) {// 幂等处理
			if (model.getOil() < model_data.getOil()
					...
					&& model_data.getCurrentCar()) { // 当前油量<数据库油量 并且
														// 数据库油量剩余不足20%
				if (model.getOil() % model_data.getOilBox()
						* 0.2 < model_data.getOil() % 5
						|| (model.getOil() < model_data.getOilBox() * 0.2
								&& model.getOil() > model_data.getOilBox()
										* 0.15)) { // 避免多次发送信息，每降低5个单位量的油量就通知车主一次
					User user = userService.findOne(model_data.getUserId());
	...
	private void monitorMileage(Car model, Car model_data) {
		if (model.getMileage() != model_data.getMileage()) {// 幂等处理
			if (model.getMileage() > model_data.getMileage()
					&& (model.getMileage() % 15000 < model_data.getMileage()
							% 15000)
	...
	private void sendMessageForUser(Car model, TcpMessage tcpMessage) {
		...
		jpushForUser.sendPush_Alias(model.getUserId().toString(),
				tcpMessage.getMessage(), tcpMessage.getTitle(), tmpMessage);
	}
	...
	public int saveCar(Car car) {
		...
		if (carList != null) {
			...
			if (backId != 0)
				carDao.updateUserCurrentCar(getDao().findOneById(backId));
			return backId;
		} else {
...
//		car.setCarState(!carState);
		...
		try {
			jpushForCar.sendPush_Alias(car.getVin(), tcpMessage.getMessage(),
					tcpMessage.toJson());
		} catch (Exception e) {
			logger.info("消息推送失败");
			e.printStackTrace();
	...
	public void insertTem_Car(Car tem_car) {
		...
		// System.out.println("----------开始检车数据库car中是否存在车架号vin为：" +
		// tem_car.getVin() + "的车辆");
		Car car_vin = new Car();
	...
	public void sendMessage(Car model) {
		...
		if (user.getRelatedPhone() != null) {
			...
			String message = "【车联网紧急通知】 您好，用户" + user.getUsername() + "在"
					+ now_data + "，大约" + address + "附近," + " 驾驶着车牌号为："
					+ model.getPlateNumber() + " 的车辆疑似发生事故，请及时联系车主确保安全。";
			try {
	...
	// @CacheEvict(cacheNames={"currentCar"}, keyGenerator = "keyGenderator")
	// //设置缓冲失效
	...
	// @Cacheable(cacheNames={"currentCar"}, keyGenerator =
	// "keyGenderator")//自定义缓存键
