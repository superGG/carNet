...
public class CarServiceImpl extends BaseServiceImpl<Car, Car> implements
     ...
     * @param model
     */
    public Boolean updateUserCurrentCar(Car model) {
        Car current_car = new Car();
        current_car.setId(model.getUserId());
        current_car.setCurrentCar(true);
        List<Car> cars = getDao().searchAccurate(current_car);
        if (cars.size() != 0) {
            current_car = cars.get(0);
            current_car.setCurrentCar(false);
            updateByPrimaryKeySelective(current_car);
            if (model.getId() != null) {
                model.setCurrentCar(true);
